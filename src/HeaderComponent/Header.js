  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex py-4 mr-4 hover:cursor-pointer" data-testid = "test-toggle-theme" onClick={handleChange}>
          <span className="mr-1">
            <i className={`fa-solid fa-${setIcon(modeColor)}`}></i>
          </span>
          <p className="font-bold" data-testid = "current-theme">{modeColor === 'Dark Mode' ? 'Light Mode' : 'Dark Mode'}</p>
        </div>
      </div>
    </div>
  );
